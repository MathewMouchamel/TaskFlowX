...
export async function storeReminder({ userId, taskId, taskTitle, dueDate, priority }) {
  if (!isInitialized) {
  ...
  try {
    ...
    await redisClient.setEx(reminderKey, 86400 * 7, JSON.stringify(reminderData)); // Expire after 7 days
    console.log(`Reminder stored for user ${userId}, task "${taskTitle || taskId}"`);
  ...
  try {
    ...
    for (const key of keys) {
      try {
        ...
        console.log(
          `${urgencyIcon} "${taskTitle || taskId}" - ${priorityIcon} ${priority?.toUpperCase()} - ${dueDateInfo}`
        );
